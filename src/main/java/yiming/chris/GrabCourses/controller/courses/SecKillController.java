...
public class SecKillController implements InitializingBean {
    ...
    public String asyncSecKill(Model model, Student student, @RequestParam("coursesId") Long coursesId) {
        ...
        Long result = redisTemplate.execute(
                SECKILL_SCRIPT,
                Arrays.asList(CoursesKey.courseStockKey.getPrefix() + ":" + coursesId.toString(),
                        CoursesKey.courseOrderKey.getPrefix() + ":" + coursesId.toString()),
                userId.toString()
        );
        ...
        /** 这里会出现超抢问题，改用Lua脚本解决
         ...
         // 如果库存不足，则直接返回秒杀失败
         if ((Integer)redisTemplate.opsForValue().get(CoursesKey.courseStockKey.getPrefix() + ":" + coursesId) < 1) {
         // 记录当前商品秒杀完毕
